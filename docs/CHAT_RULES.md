...
6. **Confirm destructive actions.** "Delete all knowledge," "delete last transaction," and similar commands must be explicit, deliberate user actions — not something ALMAS suggests or performs on its own. Voice input is never allowed to trigger a destructive action directly — ALMAS blocks it and asks the user to type the command instead. This also applies to any action the AI Intent Analyzer / Action Planner identifies (see `ARCHITECTURE.md`): destructive actions always require explicit typed confirmation and are rejected outright when they come from voice, regardless of how confident the AI is.
